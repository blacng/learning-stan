...
```{r load-data}
library(here)
pest_data <-  readRDS(here("data/pest_data.RDS")) 
head(pest_data)
str(pest_data)
```
...
fitted_model_NB_hier_slopes <-
  sampling(
    ...
    #adapt delta acceptance rate you want to target for the propoposals
    #as you move closer to 1 it forces the sampler to take smaller steps
  )
```
...
`se_mean` --- the Monte Carlo standard error. It tells us how precisely you have estimated the mean. 

`n_eff` --- the number of effective sample sizes. 
1. What's an effective sample size? 
2. If sample sizes are disparate, what does that indicate? 
...
# Having problem fitting the model using the option cores = 2, 3, 4
# This is the first time I am encountering this error
```
...
ggplot(data = profit_df, aes(x = traps, y = profit)) +
  ...
  geom_line() +
  facet_wrap(~ building, scales = 'free_y', ncol = 2) + ggthemes::theme_economist_white()
```
